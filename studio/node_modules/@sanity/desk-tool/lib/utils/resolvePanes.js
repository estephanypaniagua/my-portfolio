...
var fallbackEditorChild = (nodeId, _, _ref) => {
...
function resolvePanes(pane, paneGroups, previousPanes, fromIndex) {
  var options = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};
  var pane$ = (0, _typePredicates.isSubscribable)(pane) ? (0, _rxjs.from)(pane) : (0, _rxjs.of)(pane);
  return pane$.pipe((0, _operators.switchMap)(resolvedPane => resolveForStructure(resolvedPane, paneGroups, previousPanes, fromIndex, options)));
}

function getInitialPanes(previousPanes, numPanes, fromIndex) {
  ...
  var remains = previousPanes.slice(0, fromIndex);
  ...
  return _rxjs.Observable.create(observer => {
    ...
    function resolve(index, splitIndex) {
      ...
      for (var i = splitIndex; i < siblings.length; i++) {
        ...
        var child = isFallbackEditor ? fallbackEditorChild : parent.child;
    ...
    function getResolverArgumentsForSibling(sibling, context, isFallbackEditor) {
      var id = sibling.id,
          _sibling$params = sibling.params,
          params = _sibling$params === void 0 ? {} : _sibling$params,
    ...
    function subscribeForUpdates(pane, groupIndex, splitIndex, context, resolverArgs) {
      var source = (0, _serializeStructure.default)(pane, context, resolverArgs);
      subscriptions.push(source.subscribe(result => emit(result, groupIndex, splitIndex), error => observer.error(error)));
    }
...
var maybeSerialize = structure => structure && 'serialize' in structure ? structure.serialize({
  parent: null,
  path: [],
  index: 0,
  splitIndex: 0
...
var loadStructure = () => {
  ...
  return (0, _rxjs.defer)(() => (0, _serializeStructure.default)(structure, rootContext, ['root', rootContext]));
};
...
function isStructure(structure) {
  ...
  if (!(0, _typePredicates.isRecord)(structure)) return false;
